...
class GPUThread(threading.Thread):
    def __init__(self, number, some_array):
        ...
        self.name = 'GPUThread-' + str(number)
        self.initialized = False
        self.number= number
        self.some_array = some_array
        print('GPUThread __init__ in thread : {}'.format(threading.current_thread().name))
    ...
    def initialize(self):
        if not self.initialized:
            ...
            print('GPUThread initialize in thread : {}'.format(threading.current_thread().name))

    def run(self):
        for i in range(3):
            print('GPUThread run in thread : {}'.format(threading.current_thread().name))
            self.initialize()
